...
export default class Step1 extends Component {
    constructor(props) {
        ...
        let token = storedPage0.token;
        let locationId = storedPage0.locationId;
        let email = storedPage0.email;
        this.state = {
            data: {
                id: 1,
                to: "",
    ...
    _setData(stateId, e){
        ...
        if(elem.tagName == "SELECT"){
            if(jQuery(elem).val() != "")
            {
                ...
                this.state.data.payload[stateId].isValid = true;
            }else{
    ...
    _handleBtnClick(elem,event){
       ...
       // debugger;
        ...
        this._handleGeoValidation(this);
    ...
    _getLocationData(_self, suggest){
        if(typeof suggest == "string"){
            _self.state.data.payload.businessAddress.value = suggest;
        }else {
            _self.state.data.payload.businessAddress.value = suggest.label;
            _self.state.data.payload.addressDetails.placeId = suggest.placeId;
            _self.state.data.payload.addressDetails.lat = typeof suggest.location.lat != "undefined" ? suggest.location.lat : "";
            _self.state.data.payload.addressDetails.lng = typeof suggest.location.lng != "undefined" ? suggest.location.lng : "";
        }
        _self._handleGeoValidation(_self);
        _self.setState({data:_self.state.data});
    }
    _handleGeoValidation(_self){
        let isValid = true;
        if(jQuery("#geoInput").val() == ""){
            isValid = false;
        }
        _self.state.data.payload.businessAddress.isValid = isValid;
        _self.setState({data: _self.state.data});
//debugger;
    }
    /*   <script src="https://maps.googleapis.com/maps/api/js?libraries=places"></script>*/
    componentDidMount(){
        //let geoInput = document.getElementById('geoInput');
       // var options = {componentRestrictions: {country: 'us'}};
       // new google.maps.places.Autocomplete(input);
    }
    render(){
        let geoClassName = "inputWrap columns large-5 end";
        if(!this.state.data.payload.businessAddress.isValid){
            geoClassName += " error";
        }
                return (
                        ...
                        <form>
                            ...
                            <div className="row">
                                ...
                                <Input
                                    ...
                                    onBlur={this._handleValidation}
                                    onKeyUp={this._handleValidation}
                                ...
                                <div className={geoClassName}>
                                    <label className="geoWrap">
                                        ...
                                        <Geosuggest
                                            ...
                                            name="businessName"
                                            onBlur={this._handleGeoValidation.bind(null, this)}
                                            onChange={this._getLocationData.bind(null,this)}
                                            onSuggestSelect={this._getLocationData.bind(null,this)}
